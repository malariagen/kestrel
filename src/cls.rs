...
pub fn calculate_stacked_m(
    ...
    for v in 0..num_v {
        for g in 0..num_g {
            ...
            for ibd_mode in 0..9 {
                stacked_m[(v * num_g + g, ibd_mode)] =
                    iis::conditional_probability(pi, pj, pk, pl, iis_mode, ibd_mode + 1);
            }
        }
    }
...
pub fn calculate_quadratic_c_old(
    ...
    for (locus, (geno_x, geno_y)) in iter_x.zip(iter_y).enumerate() {
        ...
        // for ibd_mode in 0..9 {
        //     c[ibd_mode] -= iis::conditional_probability(pi, pj, pk, pl, iis_mode, ibd_mode + 1)
        // }
...
pub fn calculate_quadratic_c_t(
    all_joint_genotypes: &[((usize, usize), (usize, usize), usize)],
