...
pub fn log_avx512(d: __m512d) -> __m512d {
    ...
    e = _mm512_mask_blend_pd(mask, e, _mm512_set1_pd(1024.0));

    let m = _mm512_getmant_pd(d, _MM_MANT_NORM_P75_1P5, _MM_MANT_SIGN_NAN);

    let log2_hi = _mm512_set1_pd(0.693_147_180_559_945_286_226_764);
    let log2_lo = _mm512_set1_pd(2.319_046_813_846_299_558_417_771_e-17);
    let mut s = mul((log2_hi, log2_lo), e);

    let u = add_as_doubled(_mm512_set1_pd(-1.0), m);
    let l = add_as_doubled(_mm512_set1_pd(1.0), m);
    let x = div(u, l);
    ...
    s = add_checked_double(s, (_mm512_mul_pd(x.0, two), _mm512_mul_pd(x.1, two)));
    s = add_checked(s, _mm512_mul_pd(_mm512_mul_pd(x2, x.0), t));

    let r = _mm512_add_pd(s.0, s.1);

    let control: i64 = (4 << (2 * 4)) | (3 << (4 * 4)) | (5 << (5 * 4)) | (2 << (6 * 4));
